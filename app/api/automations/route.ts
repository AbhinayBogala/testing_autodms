...
export async function POST(
  ...
  try {
    ...
    const replyTexts = Array.from(
      new Set(
        (Array.isArray(body?.replyTexts)
          ? body.replyTexts
          : Array.isArray(body?.reply_texts)
            ? body.reply_texts
            : replyText
              ? [replyText]
              : []
    ...
    const followupDelayMinutes =
      Math.min(
        ...
        Math.max(
          60,
          Number(body?.followupDelayMinutes ?? 360) || 360,
        ),
      );

    const followupMessage =
      typeof body?.followupMessage === "string"
        ? body.followupMessage.trim().slice(0, 2000)
        : null;

    if (followupEnabled && (!buttonName || !buttonUrl || !followupMessage)) {
      return NextResponse.json(
        {
          error:
            "Follow-up requires a Custom DM Button name, URL, and follow-up message.",
        },
        { status: 400 },
      );
    }
      ...
      automationRows = [
        {
          ...
          followup_message:
            followupEnabled ? followupMessage : null,
