  ...
  try {
    ...
    const {
      ...
      .insert({
        ...
        followup_enabled:
          Boolean(source.followup_enabled),

        followup_delay_minutes:
          source.followup_delay_minutes ?? 360,

        followup_message:
          source.followup_message ?? null,
        ...
        // Preserve every public reply from the source automation.
        // Older automations may only have the legacy first reply.
        reply_texts:
          Array.isArray(source.reply_texts) && source.reply_texts.length > 0
            ? source.reply_texts
            : source.reply_text
              ? [source.reply_text]
              : [],
