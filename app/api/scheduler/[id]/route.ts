  ...
  try {
    ...
    const isScheduledPostPatch =
      ...
      (
        ...
        "mediaItems" in schedulerPatchBody ||
    ...
    if (isScheduledPostPatch) {
      try {
        ...
        if ("mediaItems" in body) {
          const rawItems = Array.isArray(body.mediaItems) ? body.mediaItems : [];
          const items = rawItems
            .map((item) => {
              ...
              const source = typeof value.source === "string" && value.source.trim().toLowerCase() === "google_drive" ? "google_drive" : "upload";
              if (!url || (type !== "image" && type !== "video")) return null;
              return { url, type, name: typeof value.name === "string" ? value.name.trim() : null, source };
            })
            .filter(Boolean) as Array<{ url: string; type: "image" | "video"; name: string | null; source: "upload" | "google_drive" }>;

          if (!items.length) {
            return NextResponse.json({ error: "At least one media item is required." }, { status: 400 });
          }
          if (items.length > 10) {
            return NextResponse.json({ error: "Instagram carousel posts support up to 10 images." }, { status: 400 });
          }
          if (items.some((item) => item.type === "video") && items.length !== 1) {
            return NextResponse.json({ error: "A Reel must contain exactly one video." }, { status: 400 });
          }

          scheduledUpdates.media_items = items;
          scheduledUpdates.media_url = items[0].url;
          scheduledUpdates.media_type = items[0].type;
          scheduledUpdates.post_type = items[0].type === "video" ? "reel" : "post";
        }
