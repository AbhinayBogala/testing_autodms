  ...
  try {
    ...
    const mediaItems = rawMediaItems
      .map((item) => {
        ...
        const source = cleanString(value.source).toLowerCase();
        if (!url || (type !== "image" && type !== "video")) return null;
        return {
          url,
          type,
          name: cleanString(value.name) || null,
          source: source === "google_drive" ? "google_drive" : "upload",
        };
      })
    ...
    const effectiveMediaItems = mediaItems.length
      ? mediaItems
      : mediaUrl
        ? [{ url: mediaUrl, type: mediaType as "image" | "video", name: null, source: "upload" as const }]
        : [];
    ...
    if (!mediaUrl && effectiveMediaItems.length === 0) {
      return jsonError(
        "Please upload at least one image or video.",
      );
    }

    if (effectiveMediaItems.length > 10) {
      return jsonError("Instagram carousel posts support up to 10 images.");
    }

    if (effectiveMediaItems.some((item) => item.type === "video") && effectiveMediaItems.length !== 1) {
      return jsonError("A Reel must contain exactly one video.");
    }

    if (effectiveMediaItems.some((item) => item.type !== "image") && postType !== "reel") {
      return jsonError("Video media must be published as a Reel.");
    }
    ...
    const insertRow = {
      ...
      media_url: effectiveMediaItems[0]?.url ?? mediaUrl,
      media_type: effectiveMediaItems[0]?.type ?? mediaType,
