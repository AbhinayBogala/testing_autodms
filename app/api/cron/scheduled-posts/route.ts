...
async function processScheduledPost(
  ...
  if (
    ...
    throw new Error(
      `Invalid scheduled post: post_type "${post.post_type}" requires video media, but media_type is "${post.media_type}".`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Invalid scheduled post: an image cannot be published as a Reel.",
    );
  }

  /*
   * A video is published as a Reel.
   * An image is published as a normal image post.
   */
  const isReel =
    isVideo;
  ...
  try {
    ...
    for (
      ...
      try {
        ...
        if (!currentPost) {
          console.log(
            "SKIPPING POST BECAUSE IT IS NO LONGER SCHEDULED:",
            post.id,
          );
