...
export default async function EditAutomationPage({
  ...
  async function updateAutomation(
    ...
    const followupEnabled =
      formData.get("followup_enabled") === "on";

    const followupDelayMinutes = Math.min(
      1380,
      Math.max(60, Number(formData.get("followup_delay_minutes") ?? 360) || 360)
    );

    const followupMessage = String(
      formData.get("followup_message") ?? ""
    ).trim();
    ...
    if (followupEnabled && (!buttonName || !buttonUrl)) {
      redirect(
    ...
    const {
      ...
      .update({
        ...
        followup_delay_minutes:
          followupEnabled ? followupDelayMinutes : 360,
    ...
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <div className="mx-auto max-w-3xl px-6 py-9">
        ...
        {!account ? (
          ...
          <div className="rounded-[26px] border border-yellow-500/15 bg-yellow-500/[0.04] p-5 sm:p-8">
          ...
          >
            ...
            <div className="mt-6 rounded-[22px] border border-white/[0.07] bg-white/[0.02] p-5">
              <label className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-medium">Follow up if the link isn&apos;t opened</p>
                  <p className="mt-1 text-xs leading-5 text-gray-500">
                    Send one reminder only when the tracked Custom DM Button has not been clicked.
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="mt-5 space-y-4">
                <div>
                  <label htmlFor="followup_delay_minutes" className="mb-2 block text-sm font-medium">Send reminder after</label>
                  <select
                    id="followup_delay_minutes"
                    name="followup_delay_minutes"
                    defaultValue={String(automationData.followup_delay_minutes ?? 360)}
                    className="w-full rounded-xl border border-white/[0.08] bg-[#070707] px-4 py-3 text-sm text-white outline-none focus:border-[#ff1744]/40"
                  >
                ...
                <div>
                  <label htmlFor="followup_message" className="mb-2 block text-sm font-medium">Follow-up message</label>
                  <textarea
                    id="followup_message"
              ...
              <p className="mt-3 text-[11px] leading-5 text-gray-600">
                Link tracking works with the Custom DM Button URL.
              </p>
            </div>
